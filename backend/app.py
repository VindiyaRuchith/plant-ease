...
def preprocess_image(image_path, target_size=(224, 224)):
    ...
    try:
        ...
        img_array /= 255.0
        logging.debug(f"Image preprocessed: {image_path}")
...
def hires_cam(model, img_array, class_index):
    ...
    try:
        ...
        # **HiRes-CAM specific step: Clip to positive gradients**
        grads = tf.nn.relu(grads)  # Retain only positive gradients

        # **HiRes-CAM specific step: Compute pixel-wise attribution**
        heatmap = tf.reduce_sum(grads * conv_outputs, axis=-1).numpy()[0]
        ...
        # Resize the heatmap to match the input size (224x224)
...
def classify_image():
    try:
        ...
        # Load the original image as RGB for blending
